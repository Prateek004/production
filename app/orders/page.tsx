        ...
        <div className="px-4 py-4 space-y-4">
          ...
          <div className="flex bg-white rounded-xl p-1 gap-1 shadow-sm">
            {(["today", "all"] as const).map((f) => (
              <button key={f} onClick={() => setFilter(f)}
                className={`flex-1 py-2 rounded-lg text-sm font-bold transition-all press ${filter === f ? "bg-primary-500 text-white shadow-sm" : "text-gray-500"}`}>
          ...
          {displayed.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-16 text-gray-300">
              ...
              {filter === "today" && <p className="text-sm mt-1">Start billing from the POS tab</p>}
            </div>
          ) : (
              ...
              {displayed.map((order) => {
                ...
                return (
                  <div key={order.id} className="bg-white rounded-2xl shadow-sm overflow-hidden">
                    <button onClick={() => setExpanded(isExp ? null : order.id)} className="w-full flex items-center px-4 py-3 press">
                        ...
                        <p className="text-xs text-gray-400 mt-0.5">
                          ...
                          {isSupabaseEnabled() && (
                            <span className="ml-2 inline-flex items-center">
                              {order.syncStatus === "synced" ? <Cloud size={10} className="text-green-400" /> : <CloudOff size={10} className="text-orange-400" />}
                            </span>
                          )}
                        </p>
                      </div>
                      <span className="font-black text-primary-500 mr-2">{fmtRupee(order.totalPaise)}</span>
                    ...
                    {isExp && (
                      <div className="px-4 pb-4 border-t border-gray-50 pt-3 fade-in">
                            ...
                            return (
                              <div key={item.cartId} className="flex justify-between text-sm">
                                <div>
                                  ...
                                  {item.selectedSize && <span className="text-xs text-gray-400 ml-1">({item.selectedSize})</span>}
                                  <span className="text-gray-400"> × {item.qty}</span>
                                </div>
                                <span className="font-semibold text-gray-900">{fmtRupee(line)}</span>
                              </div>
                            );
                          })}
                        </div>
                        <div className="border-t border-gray-100 pt-2 space-y-1 text-xs text-gray-500">
                          ...
                          {order.gstPaise > 0 && <div className="flex justify-between"><span>GST ({order.gstPercent}%)</span><span>{fmtRupee(order.gstPaise)}</span></div>}
                          ...
                          {order.splitPayment && (
                            <div className="text-xs text-gray-400 mt-1">
                              Split: Cash {fmtRupee(order.splitPayment.cashPaise)} + UPI {fmtRupee(order.splitPayment.upiPaise)}
                            </div>
                          )}
                        </div>
                      </div>
